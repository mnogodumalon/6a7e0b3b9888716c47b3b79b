...
import { DokumentenpruefungViewDialog } from '@/components/dialogs/DokumentenpruefungViewDialog';
...
const DOKUMENTENPRUEFUNG_FIELDS = [
  { key: 'titel', label: 'Dokumententitel', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'dokument', label: 'Dokument', type: 'file' },
  { key: 'hochladedatum', label: 'Hochladedatum', type: 'date/date' },
  { key: 'pruefstatus', label: 'Prüfstatus', type: 'lookup/radio', options: [{ key: 'bestanden', label: 'Bestanden' }, { key: 'nicht_bestanden', label: 'Nicht bestanden' }, { key: 'ausstehend', label: 'Ausstehend' }, { key: 'nachbesserung_erforderlich', label: 'Nachbesserung erforderlich' }] },
  { key: 'pruefkommentar', label: 'Prüfkommentar', type: 'string/textarea' },
  { key: 'prueferdatum', label: 'Prüfdatum', type: 'date/date' },
  { key: 'pruefer', label: 'Prüfer', type: 'string/text' },
];

const ENTITY_TABS = [
  { key: 'dokumentenpruefung', pascal: 'Dokumentenpruefung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('dokumentenpruefung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'dokumentenpruefung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'dokumentenpruefung': return (data as any).dokumentenpruefung as Dokumentenpruefung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'dokumentenpruefung': return DOKUMENTENPRUEFUNG_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'dokumentenpruefung': return {
        create: (fields: any) => LivingAppsService.createDokumentenpruefungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateDokumentenpruefungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteDokumentenpruefungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'dokumentenpruefung' || dialogState?.entity === 'dokumentenpruefung') && (
        <DokumentenpruefungDialog
          open={createEntity === 'dokumentenpruefung' || dialogState?.entity === 'dokumentenpruefung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'dokumentenpruefung' ? handleUpdate : (fields: any) => handleCreate('dokumentenpruefung', fields)}
          defaultValues={dialogState?.entity === 'dokumentenpruefung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Dokumentenpruefung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Dokumentenpruefung']}
        />
      )}
      {viewState?.entity === 'dokumentenpruefung' && (
        <DokumentenpruefungViewDialog
          open={viewState?.entity === 'dokumentenpruefung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'dokumentenpruefung', record: r }); }}
        />
      )}
