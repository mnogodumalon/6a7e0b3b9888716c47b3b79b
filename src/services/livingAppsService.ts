...
import type { Dokumentenpruefung, CreateDokumentenpruefung } from '@/types/app';
...
export class LivingAppsService {
  // --- DOKUMENTENPRUEFUNG ---
  static async getDokumentenpruefung(): Promise<Dokumentenpruefung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.DOKUMENTENPRUEFUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Dokumentenpruefung[];
    return enrichLookupFields(records, 'dokumentenpruefung');
  }
  static async getDokumentenpruefungEntry(id: string): Promise<Dokumentenpruefung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.DOKUMENTENPRUEFUNG}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Dokumentenpruefung;
    return enrichLookupFields([record], 'dokumentenpruefung')[0];
  }
  static async createDokumentenpruefungEntry(fields: CreateDokumentenpruefung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.DOKUMENTENPRUEFUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'dokumentenpruefung') });
    return { ...data, record_id: data.id };
  }
  static async updateDokumentenpruefungEntry(id: string, fields: Partial<CreateDokumentenpruefung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.DOKUMENTENPRUEFUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'dokumentenpruefung') });
    return { ...data, record_id: data.id };
  }
  static async deleteDokumentenpruefungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.DOKUMENTENPRUEFUNG}/records/${id}`);
  }
